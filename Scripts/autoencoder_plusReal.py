...
#    autoencoder.append(Model(input_dim, decoder))
...
for k in range(len(x_train)): 
    ...
    rec_error_threshold_LEVEL0 = (errors_train[k].max()+0.25*errors_train[k].std())[0]

    rec_error_threshold_LEVEL1 = (errors[k].max()-1.5*errors[k].std())[0]
    rec_error_threshold_LEVEL2 = (errors[k].max()-1*errors[k].std())[0] 
...
for i in range(len(mat)):
    fig, axs = plt.subplots(1)
    im = axs.imshow(mat[i],aspect=0.005*plot_scaler*max(mat[i].shape)
...
for k in range(len(mat)): 
    predictions_real = autoencoder[k].predict(mat[k])
    mse_real = np.mean(np.power(mat[k] - predictions_real, 2), axis=1)
